...
const ffmpegPath = `${HOME}/ffmpeg`;
const youtubeDLPath = `${HOME}/youtube-dl`;
...
const executableArgs = [
  ...
  '-x', // --extract-audio
  '--audio-format=mp3',
  // 0-9: VBR | 0: best quality | 9: worse quality
  '--audio-quality=320K',
  '--embed-thumbnail',
  // SUBTITLES
  // '--all-subs',
...
function isFileExecutable(path) {
  return new Promise((resolve, reject) => {
    access(path, X_OK, err =>
      err ? console.error(err, path) || reject(err) : resolve()
    );
  });
}
...
function isFileReadable(path) {
  return new Promise(resolve => {
    access(path, R_OK, err =>
      err
        ? (env === 'development' && console.error(err, path)) || resolve(false)
        : resolve(true)
    );
  });
}
